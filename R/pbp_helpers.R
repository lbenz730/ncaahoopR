...
clean <- function(data) {
  ...
  half <- cleaned$period.number
  OTs <- max(cleaned$period.number) - 2
  ...
  cleaned <- 
    cleaned %>% 
    dplyr::select(half, time_remaining_half, secs_remaining, description, home_score, away_score, scoring_play, action_team)
