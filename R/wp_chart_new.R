...
wp_chart_new <- function(game_id, home_col = NULL, away_col = NULL, include_spread = T, show_labels = T) {
  ...
  home_url <- ncaa_colors$logo_url[ ncaa_colors$espn_name == dict$ESPN[dict$ESPN_PBP == home_team | dict$ESPN_PBP == gsub('State', 'St', home_team)][1] ]
  away_url <- ncaa_colors$logo_url[ ncaa_colors$espn_name == dict$ESPN[dict$ESPN_PBP == away_team | dict$ESPN_PBP == gsub('State', 'St', away_team)][1] ]
